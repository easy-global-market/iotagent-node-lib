...
const multiEntityPlugin = require('../../plugins/multiEntity')
const context = {
...
function createInitialEntityHandlerNgsiLD(deviceData, newDevice, callback) {
    return function handleInitialEntityResponse(error, response, body) {
        if (error) {
            ...
            callback(error);
        } else if (response && (response.statusCode === 204 || response.statusCode == 200)) {
            // FIXME why do we have this??
            if (body && body.errors && body.errors.length > 0) {
                logger.error(
                    context,
                    'Protocol error connecting to the Context Broker [%d]: %j',
                    response.statusCode,
                    body
                );
...
function createInitialEntityNgsiLD(deviceData, newDevice, callback) {
    function jsonDeviceData(deviceData) {
        let json = {
    ...
    var json = jsonDeviceData(deviceData);


    multiEntityPlugin.update(json, deviceData, function (error, entities, typeInformation) {
        if (error) {
            return callback(error);
        }

        if (entities instanceof Array) {
            json = [ngsiLD.formatAsNGSILD(entities[0])]
            deviceId = json[0].id
            entities.slice(1)
                .map(ngsiLD.formatAsNGSILD)
                .forEach(entity => json.push(entity));
        } else {
